...
QUESTIONS = [
        {
        "text": "Give your initials and your favorite number (Keep this the same)",
        "type": "open"
    },
    {
        "text": "What is your sex? (Please respond with \"male\" or \"female\")",
        "type": "open"
    },
    {
        "text": "What is your age? (Please respond with a number, not date of birth)",
        "type": "open"
    },
    {
        "text": "How is your English proficiency?",
        "type": "rating"
    },
    {
        "text": "How natural was your conversation with KASA?",
        "type": "rating"
    },
    {
        "text": "How clear were the answers and questions from KASA?",
        "type": "rating"
    },
    {
        "text": "What was the quality of the conversation with KASA?",
        "type": "rating"
    },
    {
        "text": "How human-like were the answers and questions of KASA?",
        "type": "rating"
    },
    {
        "text": "How would you rate your experience using KASA for a restaurant recommendation?",
        "type": "rating"
    },
    {
        "text": "How likely are you to recommend KASA?",
        "type": "rating"
    },
    {
        "text": "Did you experience any difficulties during your conversation? If so, what happened?",
        "type": "open"
    },
]
...
def state_transition(utterance, state, act, preferences, restaurants, result, additional_pref, formal, convo, t0):
    response = ''
                ...
                if(len(additional_pref) == 0):
                    ...
                    if result == None: # if no restaurants matches go to state 6 and ask wheter user want to restart
                        ...
                        if formal:
                            ...
                            response = "Thankss, I came up with a recommendation: {}. {} Sounds good???".format(result, reason)
    ...
    #State 9 is ending the conversation after you give a rating to the bot
        ...
        if SURVEY_INDEX > 0:
            # Add response to previous question to SURVEY_RESPONSE
            SURVEY_RESPONSES.append(utterance)
        if SURVEY_INDEX == len(QUESTIONS): # Survey is done

            #add formality True or False whether it is used
            SURVEY_RESPONSES.append(str(formal))

            # add convo length as one before last column in survey
            SURVEY_RESPONSES.append(str(len(convo)-len(QUESTIONS)-1))
            # add convo time as last column (s)
            SURVEY_RESPONSES.append(str(time.time() - t0))

            new_row = ','.join(SURVEY_RESPONSES)

            with open('survey.csv', 'a') as f:
                f.write(new_row)
                f.write('\n')

            response = "Thank you very much! To exit, please click \"Enter\" " #send some thankyou message
            ...
            # Get next questions
            response = get_survey_question(SURVEY_INDEX)

            # updat global var SURVEY_INDEX
            SURVEY_INDEX += 1
            new_state = state
...
def get_survey_question(index):
    ...
    if question['type'] == 'rating':
        text = question['text'] + ' (Please respond with an number between 1-5; where 1 is \"very bad\" and 5 is \"very good\")'
    elif question['type'] == 'open':
        text = question['text'] + ' (Please respond with your answer in free text)'
    elif question['type'] == 'yesno':
        text = question['text'] + ' (Please respond with \"yes\" or \"no\")' 
    else:
        raise NotImplementedError("Question type '%s' not recognized.")
    
    return text


#function that retrieves and saves the additional requirements
            ...
            if consequent == "busy":
                ...
                if (list(rest_info['type'])[indexje] == "spanish"):
                    if formal:
                        ...
                        reason = "You can spend a long time in this restaurant, because Spanish restaurants serve dinners that will take you sooooo long to finish."
                    result = restaurant
                    restaurants.remove(restaurant)
                    return reason, result, restaurants
                ...
                if (list(rest_info['staytime'])[indexje] == "false"):
                    if formal:
                        ...
                        reason = "This restaurant will be sooo perfect when you bring children, because you don't have to spend a long time in this restaurant!!!"
                    result = restaurant
                    restaurants.remove(restaurant)
                    return reason, result, restaurants
                ...
                if (list(rest_info['crowdedness'])[indexje] == "false"):
                    ...
                    if formal:
                        ...
                        reason = "This one is perfect for some romance, because you can spend such a loooong time in this restaurant :)"
                    result = restaurant
                    restaurants.remove(restaurant)
                    return reason, result, restaurants
...
def give_response(state, formal, preferences):
    ...
    if formal:
        responses ={
                ...
                "survey": "I want to keep improving. Do you mind filling in a quick survey? (say anything to continue)",
    ...
    #template fr informal responses
        ...
        responses ={
                ...
                "survey": "I hope you liked our conversation, I sure did! Do you mind filling in a quick survey? (say anything to continue)",
...
def give_recommendation(restaurants, formal):
    ...
    if formal:
        ...
        response = "Okayy, I came up with a sweet recommendation: {}. Sounds good??? ".format(recommendation)
